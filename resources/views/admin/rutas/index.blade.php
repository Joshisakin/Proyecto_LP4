                ...
                <table class="table table-bordered" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $ruta->origen }}</td>
                            <td>{{ $ruta->destino }}</td>
                            <td>{{ $ruta->fecha_salida->format('d/m/Y H:i') }}</td>
                            <td>{{ number_format($ruta->duracion, 1) }} hrs</td>
                            <td>S/ {{ number_format($ruta->precio, 2) }}</td>
                            <td>{{ $ruta->capacidad }}</td>
                            <td>
                                <span class="badge bg-success">{{ $ruta->capacidad }}</span>
                            </td>
                            <td>
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" role="switch"
                                           id="estado{{ $ruta->id }}"
                                           {{ $ruta->estado ? 'checked' : '' }}
                                           onchange="cambiarEstado({{ $ruta->id }})">
                                </div>
                            </td>
                            <td>
...
@push('scripts')
<script>
function cambiarEstado(rutaId) {
    const checkbox = document.getElementById('estado' + rutaId);
    const estado = checkbox.checked;

    // Aquí puedes agregar la lógica para actualizar el estado mediante AJAX
    fetch(`/admin/rutas/${rutaId}/toggle-status`, {
        method: 'POST',
        headers: {
